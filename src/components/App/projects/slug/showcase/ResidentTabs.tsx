...
function ResidentCardContent({name, status, type, area, isActive, onClick}: ResidentProps) {
  return (
    <div onClick={onClick} className={cn('p-2.5 xl:pb-1 flex flex-col gap-1.5 xl:gap-1 duration-200 cursor-pointer', isActive ? 'bg-red text-background' : 'bg-background hover:bg-blue group hover:text-background')}>
  ...
  return (
    <div className="relative flex max-w-[55vw] xl:max-w-[60vw] gap-1.5">
      {residents.length > 3 ? (
          ...
          <Swiper ref={swiperRef} spaceBetween={6} slidesPerView="auto" centeredSlides={false} className="!w-full">
            {residents.map(({name, status, type, area}, index) => (
              <SwiperSlide key={index} className="!w-auto flex-shrink-0">
                <ResidentCardContent name={name} status={status} type={type} area={area} isActive={activeTab === index} onClick={() => handleTabClick(index)} />
              </SwiperSlide>
            ))}
          </Swiper>
        ...
        residents.map(({name, status, type, area}, index) => <ResidentCardContent key={index} name={name} status={status} type={type} area={area} isActive={activeTab === index} onClick={() => handleTabClick(index)} />)
      )}
    </div>
  )
}
